...
local function OpenInventory(name, id, other, origin)
	...
	if name and id then
		...
		TriggerClientEvent("0x-inventory:client:closeinv", id)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                local imageurl = ("https://cfx-nui-0x-inventory/html/images/%s.png"):format(itemData.name)
...
RegisterNetEvent('0x-inventory:server:SaveStashItems', function(stashId, items)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, item.created) then
				...
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('0x-inventory:client:giveAnim', src)
				TriggerClientEvent('0x-inventory:client:giveAnim', target)
			else
...
QBCore.Functions.CreateCallback('0x-inventory:server:GetStashItems', function(source, cb, stashId)
...
RegisterNetEvent('inventory:server:addTrunkItems', function()
	print('inventory:server:addTrunkItems has been deprecated please use exports[\'0x-inventory\']:addTrunkItems(plate, items)')
end)
RegisterNetEvent('inventory:server:addGloveboxItems', function()
	print('inventory:server:addGloveboxItems has been deprecated please use exports[\'0x-inventory\']:addGloveboxItems(plate, items)')
end)
