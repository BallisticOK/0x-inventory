...
function showBlur() {
    $.post('https://0x-inventory/showBlur');
}
...
//     localStorage.setItem('0x-inventory-color', color);
// }

// const savedColor = localStorage.getItem('0x-inventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://0x-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://0x-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://0x-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://0x-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://0x-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://0x-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://0x-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (
                    ...
                    $.post(
                        "https://0x-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://0x-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://0x-inventory/CloseInventory", JSON.stringify({}));
